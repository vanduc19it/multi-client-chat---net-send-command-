			...
			public void actionPerformed(ActionEvent e) {
				...
				if (textAreaMessage != null && !textAreaMessage.isEmpty()) {  // only if message is not empty then send it further otherwise do nothing
					...
					try {
						...
						boolean netsend = textAreaMessage.contains("net send {*}"); //kiem tra text chưa net send * khong
						boolean netsend1 = textAreaMessage.contains("net send {group}");
						
						
						String a = "";
						for(int i = 10; i < textAreaMessage.length(); i++)  {//lay text trong {....}
							
							a += textAreaMessage.charAt(i);  
							boolean c = a.contains("}");
							if(c == true) break;
						}
						 String b= "";
					for(int i = 0; i < a.length()-1; i++)  { /// loaij bo dau }
							
							b += a.charAt(i);  
							
						}
							
						 boolean netsend2 = textAreaMessage.contains("net send {" + b + "}");
						
						String cast1 = "broadcast";
						if(netsend == true) { //netsend {*}
							
							String[] parts = textAreaMessage.split("}");
							String part1 = parts[0]; 
							String part2 = parts[1]; 
							messageToBeSentToServer = cast1 + ":" + part2;
							outStream.writeUTF(messageToBeSentToServer);
							clientTypingBoard.setText("");
							clientMessageBoard.append("< You sent msg to All >" + part2 + "\n");				
						} 
						else if(netsend1 == true){   //netsend {group}
							String[] parts = textAreaMessage.split("}");
							String part1 = parts[0]; 
							String part2 = parts[1]; 
							if (oneToNRadioBtn.isSelected()) { // if 1-to-N is selected then do this
								...
								for (String selectedUsr1 : clientList) { // append all the usernames selected in a variable
									if (clientIds.isEmpty())
										clientIds += selectedUsr1;
									else
										clientIds += "," + selectedUsr1;
								}
								messageToBeSentToServer = cast1 + ":" + clientIds + ":" +part2; // prepare message to be sent to server
							}
							outStream.writeUTF(messageToBeSentToServer);
							clientTypingBoard.setText("");
							clientMessageBoard.append("< You sent msg to " + clientIds + ">" + part2 + "\n"); 
						}
						
						else if(netsend2 == true) {   //netsend {username}
							 String[] p4 = textAreaMessage.split("}"); 
								String p5 = p4[0]; 
								String p6 = p4[1];
							if (oneToNRadioBtn.isSelected()) { 
								
									messageToBeSentToServer = "multicast" + ":" + b + ":" + p6; 
							}else {
					    	  ...
					    	  outStream.writeUTF(messageToBeSentToServer);
								clientTypingBoard.setText("");
								clientMessageBoard.append("< You sent msg to " + b + ">" + p6 + "\n");
						}
						
						else { // nhan tin binh thuong
